                ...
                for generic in &generic_list.items {
                    if let InterfaceDeclaration::Object(obj_decl) = generic {
                        if let ModeIndication::Simple(simple) = &obj_decl.mode {
                            ...
                            let default = simple.expression.as_ref().map(|expression| expression.to_string());

                            for id in &obj_decl.idents {
                                let name = id.tree.item.name_utf8();
                                let description = find_object_description(tokens, id.tree.token, priority_trailing, true);
...
mod tests {
    ...
    fn test_generic_descriptions() {
        ...
        assert_eq!(generics[0].description, Some("a flag, can be true or false".to_owned()));
        ...
        assert_eq!(generics[1].description, Some("a value with a default".to_owned()));
...
fn get_generic_list(id: &[u8], file_name: &[u8], vhdl_standard: &[u8], contents: &[u8], comment_priority: &[u8]) -> Result<Vec<u8>, String> {
